...
RegisterNetEvent("housing:client:cloakroom", function()
    local clothes = QBCore.Functions.TriggerRpc("op-character:server:GetPlayerCloakroom")
    ...
    Housing.Functions.GenerateMenu(function(menu)
        menu:AddButton({
            ...
            select = function()
                local name = exports["op-hud"]:Input("Nom de la tenue", 64)

                local error = QBCore.Functions.TriggerRpc("op-character:server:SavePlayerClothe", name)
                if error == nil then
                    exports["op-hud"]:DrawNotification("Votre tenue a été sauvegardée.")
                else
                    exports["op-hud"]:DrawNotification(error, "error")
                    return
                end
                menu:Close()
            end,
                ...
                select = function(_, value)
                    if value == "apply" then
                        QBCore.Functions.Progressbar("switch_clothes", "Changement d'habits...", 5000, false, true,
                            ...
                            TriggerServerEvent("op-character:server:SetPlayerClothes", clothe.cloth)
                        end)
                    elseif value == "delete" then
                        local success = QBCore.Functions.TriggerRpc("op-character:server:DeletePlayerClothe", clothe.id)
                        if success then
                            exports["op-hud"]:DrawNotification("Votre tenue a été supprimée.")
                        else
                            exports["op-hud"]:DrawNotification("Une erreur est survenue.", "error")
                        end
                    end
                    menu:Close()
                end,
