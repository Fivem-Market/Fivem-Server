...
RegisterNetEvent("banking:openBankScreen", function()
    local currentBank = exports["op-bank"]:GetCurrentBank()
...
RegisterNUICallback("createOffshoreAccount", function(data, cb)
    QBCore.Functions.TriggerCallback("banking:server:createOffshoreAccount", function(success, reason)
        if success then
            exports["op-hud"]:DrawAdvancedNotification("Maze Banque", "Création de compte", "Vous avez crée un nouveau compte", "CHAR_BANK_MAZE")
        else
            exports["op-hud"]:DrawNotification(Config.ErrorMessage[reason], "error")
        end
...
RegisterNUICallback("doDeposit", function(data, cb)
    ...
    if amount ~= nil and amount > 0 then
        QBCore.Functions.TriggerCallback("banking:server:TransferMoney", function(success, reason)
            if success then
                exports["op-hud"]:DrawAdvancedNotification("Maze Banque", "Dépot: ~g~" .. amount .. "$", "Vous avez déposé de l'argent", "CHAR_BANK_MAZE")
            else
                exports["op-hud"]:DrawNotification(Config.ErrorMessage[reason], "error")
            end
...
RegisterNUICallback("doWithdraw", function(data, cb)
    ...
    if terminalConfig.maxWithdrawal then
        if amount > terminalConfig.maxWithdrawal then
            exports["op-hud"]:DrawNotification(string.format(Config.ErrorMessage["max_widthdrawal_limit"], terminalConfig.maxWithdrawal), "error")
            return
        end
        ...
        if lastUse ~= nil then
            ...
            if remainingTime > 0 then
                if amountAvailableForWithdraw == 0 then
                    exports["op-hud"]:DrawNotification(limit .. string.format(Config.ErrorMessage["time_limit"], math.ceil(remainingTime / 60000)), "error")
                    return
                elseif amount > amountAvailableForWithdraw then
                    exports["op-hud"]:DrawNotification(limit .. string.format(Config.ErrorMessage["withdrawal_limit"], amountAvailableForWithdraw), "error")
                    return
                end
            end
        end
    ...
    QBCore.Functions.TriggerCallback("banking:server:hasEnoughLiquidity", function(result, reason)
        if not result then
            if reason == "invalid_liquidity" then
                ...
                exports["op-hud"]:DrawNotification(Config.ErrorMessage["unknown"], "error")
            end
        end
    ...
    if amount ~= nil and amount > 0 then
        QBCore.Functions.TriggerCallback("banking:server:TransferMoney", function(success, reason)
            if success then
                exports["op-hud"]:DrawAdvancedNotification("Maze Banque", "Retrait: ~r~" .. amount .. "$", "Vous avez retiré de l'argent", "CHAR_BANK_MAZE")
...
RegisterNUICallback("doTransfer", function(data, cb)
    ...
    if amount ~= nil and amount > 0 then
        QBCore.Functions.TriggerCallback("banking:server:TransferMoney", function(success, reason)
            if success then
                exports["op-hud"]:DrawAdvancedNotification("Maze Banque", "Transfert: ~r~" .. amount .. "$", "Vous avez transféré de l'argent sur un compte",
                                                            "CHAR_BANK_MAZE")
            else
                exports["op-hud"]:DrawNotification(Config.ErrorMessage[reason], "error")
            end
