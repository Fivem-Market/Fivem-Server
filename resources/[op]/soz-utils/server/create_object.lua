...
local function CreateObjectClientSide(model, x, y, z, w, culling, freeze)
    ...
    TriggerClientEvent("op-utils:client:create-object", -1, ref, model, x, y, z, w, culling, freeze)
...
local function DeleteObjectClientSide(ref)
    for i, v in pairs(ObjectList) do
        if v.ref == ref then
            ...
            TriggerClientEvent("op-utils:client:delete-object", -1, ref)
...
QBCore.Functions.CreateCallback("op-utils:object:GetList", function(source, cb)
    cb(ObjectList)
end)
