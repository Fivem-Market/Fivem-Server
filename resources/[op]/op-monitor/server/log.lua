    ...
    while true do
        ...
        if #log_buffer_loki > 0 then
            PerformHttpRequest(log_handler_loki, function(status, text, headers)
                if status ~= 204 then
                    print("[op-Monitor] Loki log handler: " .. status .. ", " .. json.encode(headers) .. ", " .. json.encode(text))
                end
...
local function formatLogString(level, message, playerData)
    ...
    logMessage = ReplaceString(logMessage, "%emitter%", GetInvokingResource() or "op-monitor")
...
local function formatLogJson(level, message, extraData)
    ...
    logPayload.emitter = GetInvokingResource() or "op-monitor"
...
local function formatLogLoki(level, message, data)
    ...
    logPayload.stream.emitter = GetInvokingResource() or "op-monitor"
