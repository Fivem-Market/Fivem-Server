...
export class Application {
    ...
    async start() {
        if (this.running !== null && this.resolve !== null) {
            this.logger.error('op core applicasion already running');
        ...
        addEventListener('op_core.__internal__.stop_application', this.onStopCallback, false);
    ...
    async stop() {
        if (this.running === null || this.resolve === null) {
            this.logger.error('op core application is not running');
    ...
    private onStop() {
        ...
        removeEventListener('op_core.__internal__.stop_application', this.onStopCallback);
    }
}
