...
export class BankService {
    ...
    public transferBankMoney(source: string, target: string, amount: number): Promise<[boolean, string]> {
        return new Promise(resolve => {
            exports['op-bank'].TransferMoney(source, target, amount, (success, reason) => {
    ...
    public transferCashMoney(source: string, target: number, amount: number): Promise<Result<boolean, string>> {
        return new Promise(resolve => {
            exports['op-bank'].TransferCashMoney(source, target, amount, (success, reason) => {
                if (success) {
                    resolve(Ok(true));
                } else {
                    resolve(Err(reason));
    ...
    public getBank() {
        return exports['op-bank'].GetCurrentBank();
    }
