...
export class PlayerCleanService {
    ...
    async getPlayerToCleans(): Promise<string[]> {
        const playersToClean = (await this.prismaService.$queryRawUnsafe(
            `SELECT p.citizenId
             FROM op_api.accounts a
                      INNER JOIN op_api.account_identities ai ON a.id = ai.accountId AND ai.identityType = 'STEAM'
                      INNER JOIN op_fivem.player p ON ai.identityId = p.license
        ...
        const deletedPlayersToClean = (await this.prismaService.$queryRawUnsafe(
            `SELECT p.citizenId FROM op_fivem.player p LEFT JOIN op_api.account_identities ai ON p.license = ai.identityId AND ai.identityType = 'STEAM' WHERE ai.identityType IS NULL AND p.is_default = 1`
        )) as { citizenId: string }[];
