            ...
            CreateThread(function()
                ...
                if vehicle then
                    ...
                    if pedFound >= 1 then
                        ...
                        TriggerServerEvent("op-core:server:vehicle:take-owner", vehicleNetworkId)
                        exports["op-hud"]:DrawNotification("Vous venez de réquisitionner ce véhicule")
...
local function RadarEntity(menu, job)
    ...
    radarItem:On("change", function(menu, value)
        ...
        TriggerEvent("op-core:client:radar:toggle-blip", value, job)
    end)
end
    ...
    menu:AddButton({
        ...
        select = function()
            ...
            menu:AddButton({
                ...
                value = nil,
                select = function()
                    local name = exports["op-hud"]:Input("Nom de la personne recherchée :", 125)
                    if name == nil or name == "" then
                        exports["op-hud"]:DrawNotification("Vous devez spécifier un nom", "error")
                        return
                    end
            ...
            for _, wantedPlayer in pairs(wantedPlayers) do
                menu:AddButton({
                    ...
                    select = function()
                        ...
                        if deletion then
                            exports["op-hud"]:DrawNotification("Vous avez retiré ~b~" .. wantedPlayer.message .. " ~s~de la liste des personnes recherchées")
                            menu:Close()
                        end
                    end,
                })
    ...
    PoliceJob.Functions.Menu.GenerateMenu(job, function(menu)
        if PlayerData.job.id == "fbi" then
            menu:AddButton({
                ...
                value = nil,
                select = function(_, value)
                    local message = exports["op-hud"]:Input("Message de la communication", 512)
                    if message == nil or message == "" then
                        exports["op-hud"]:DrawNotification("Vous devez spécifier un message", "error")
                        return
                    end
            ...
            select = function()
                ...
                QBCore.Functions.Progressbar("job:police:fines", "Rédaction de l'amende", 5000, false, true,
                    ...
                    if #(GetEntityCoords(ped) - GetEntityCoords(GetPlayerPed(player))) < 2.5 then
                        ...
                        exports["op-hud"]:DrawNotification("Personne n'est à portée de vous", "error")
                    end
                end)
            end,
        })
            ...
            for _, fine in ipairs(finesCategory.items) do
                ...
                category:AddButton({
                    ...
                    select = function()
                        ...
                        local fineAmount
                        if type(fine.price) == "table" then
                            fineAmount = exports["op-hud"]:Input(("Montant de l'amende ($%d - $%d)"):format(fine.price.min, fine.price.max), 10)
                            if fineAmount == nil or tonumber(fineAmount) < fine.price.min or tonumber(fineAmount) > fine.price.max then
                                exports["op-hud"]:DrawNotification("Montant de l'amende invalide", "error")
                                return
                            end
                        else
