...
function getAmountFromShortcutModifier(keyModifier, amount, maxAmount)
    ...
    elseif amount > 1 and keyModifier == "ALT" then
        ...
        tempAmount = exports["op-hud"]:Input("Quantité", 5, math.floor(amount / 2))
        SetNuiFocus(true, true)
        return tempAmount
    ...
    elseif amount >= 1 then
        ...
        if tempAmount <= 0 then
            exports["op-hud"]:DrawNotification("Cet inventaire est déjà plein", "error")
            return amount
        end

        if maxAmount < amount then
            exports["op-hud"]:DrawNotification(maxAmount .. " objets déplacés", "info")
        end
...
RegisterNUICallback("closeNUI", function(data, cb)
    ...
    if data.target then
        ...
        if string.find(data.target, "trunk") or data.target == "target" then
            TriggerEvent("op-core:client:vehicle:close-trunk")
        end
    end
end)
...
RegisterNetEvent("inventory:client:qTargetOpenInventory", function(data)
    if data.storage.owner == nil or (PlayerData.job ~= nil and PlayerData.job.id == data.storage.owner) then
        ...
        exports["op-hud"]:DrawNotification("Vous ne pouvez pas utiliser ce stockage", "error")
    end
end)
