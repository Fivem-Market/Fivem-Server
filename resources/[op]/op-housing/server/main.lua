...
local function IsPropertyValid(house)
    ...
    if house.identifier == nil then
        exports["op-monitor"]:Log("DEBUG", ("Entry #%s skipped because it has no identifier"):format(house.id))
        return false
    end
    if house.entry_zone == nil then
        exports["op-monitor"]:Log("DEBUG", ("Entry %s skipped because it has no entry_zone"):format(house.identifier))
        return false
    end
    return true
end
...
local function IsApartmentValid(house)
    ...
    if house.price == nil then
        exports["op-monitor"]:Log("DEBUG", ("Entry %s skipped because it has no price"):format(house.label))
        return false
    end
    if house.inside_coord == nil then
        exports["op-monitor"]:Log("DEBUG", ("Entry %s skipped because it has no inside_coord"):format(house.label))
        return false
    end
    if house.exit_zone == nil then
        exports["op-monitor"]:Log("DEBUG", ("Entry %s skipped because it has no exit_zone"):format(house.label))
        return false
    end
    return true
end
...
RegisterNetEvent("housing:server:SetPlayerInApartment", function(propertyId, apartmentId, target)
    ...
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("EnterApartment %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:EnterApartment", function(propertyId, apartmentId)
    ...
    if not apartment:HasAccess(Player.PlayerData.citizenid) then
        exports["op-monitor"]:Log("ERROR", ("EnterApartment %s - Apartment %s | skipped because player has no access"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:ExitProperty", function(propertyId, apartmentId)
    ...
    local apartment = Properties[propertyId]:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("ExitProperty %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:InspectApartment", function(propertyId, apartmentId)
    ...
    local apartment = Properties[propertyId]:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("InspectApartment %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if not apartment:IsAvailable() then
        exports["op-monitor"]:Log("ERROR", ("InspectApartment %s - Apartment %s | skipped because it is not available"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:BellProperty", function(propertyId, apartmentId)
    ...
    local apartment = Properties[propertyId]:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("BellProperty %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if apartment:IsAvailable() then
        exports["op-monitor"]:Log("ERROR", ("BellProperty %s - Apartment %s | skipped because it is available"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:BuyApartment", function(propertyId, apartmentId)
    ...
    local apartment = Properties[propertyId]:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("BuyApartment %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if not apartment:IsAvailable() then
        exports["op-monitor"]:Log("ERROR", ("BuyApartment %s - Apartment %s | skipped because it is not available"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:SellApartment", function(propertyId, apartmentId)
    ...
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("SellApartment %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if apartment:IsAvailable() then
        exports["op-monitor"]:Log("ERROR", ("SellApartment %s - Apartment %s | skipped because it is available"):format(propertyId, apartmentId))
        return
    end

    if not apartment:IsOwner(Player.PlayerData.citizenid) then
        exports["op-monitor"]:Log("ERROR", ("SellApartment %s - Apartment %s | skipped because player has no access"):format(propertyId, apartmentId))
        return
    end
    ...
    if Player.Functions.AddMoney("money", resellPrice) then
        ...
        if apartment:HasRoommate() then
            ...
            exports["op-character"]:TruncatePlayerCloakroomFromTier(roommateCitizenId, 0)
        ...
        exports["op-inventory"]:SetHouseStashMaxWeightFromTier(apartment:GetIdentifier(), 0)
        Player.Functions.SetApartment(nil)

        exports["op-character"]:TruncatePlayerCloakroomFromTier(Player.PlayerData.citizenid, 0)
...
RegisterNetEvent("housing:server:AddRoommateApartment", function(propertyId, apartmentId, targetId)
    ...
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("AddRoommateApartment %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if apartment:IsAvailable() then
        exports["op-monitor"]:Log("ERROR", ("AddRoommateApartment %s - Apartment %s | skipped because it is available"):format(propertyId, apartmentId))
        return
    end

    if not apartment:IsOwner(Player.PlayerData.citizenid) then
        exports["op-monitor"]:Log("ERROR", ("AddRoommateApartment %s - Apartment %s | skipped because player has no access"):format(propertyId, apartmentId))
        return
    end

    if apartment:GetRoomMate() then
        exports["op-monitor"]:Log("ERROR",
...
RegisterNetEvent("housing:server:RemoveRoommateApartment", function(propertyId, apartmentId)
    ...
    local apartment = property:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("RemoveRoommateApartment %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if apartment:IsAvailable() then
        exports["op-monitor"]:Log("ERROR", ("RemoveRoommateApartment %s - Apartment %s | skipped because it is available"):format(propertyId, apartmentId))
        return
    end

    if not apartment:IsOwner(Player.PlayerData.citizenid) and not apartment:IsRoommate(Player.PlayerData.citizenid) then
        exports["op-monitor"]:Log("ERROR", ("RemoveRoommateApartment %s - Apartment %s | skipped because player has no access"):format(propertyId, apartmentId))
        return
    end

    if not apartment:GetRoomMate() then
        exports["op-monitor"]:Log("ERROR", ("RemoveRoommateApartment %s - Apartment %s | skipped because it has no roommate"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:GiveTemporaryAccess", function(propertyId, apartmentId, target)
    ...
    local apartment = Properties[propertyId]:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("GiveTemporaryAccess %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if apartment:IsAvailable() then
        exports["op-monitor"]:Log("ERROR", ("GiveTemporaryAccess %s - Apartment %s | skipped because it is available"):format(propertyId, apartmentId))
        return
    end

    if not apartment:IsOwner(Player.PlayerData.citizenid) and not apartment:IsRoommate(Player.PlayerData.citizenid) then
        exports["op-monitor"]:Log("ERROR", ("GiveTemporaryAccess %s - Apartment %s | skipped because player has no access"):format(propertyId, apartmentId))
        return
    end
...
RegisterNetEvent("housing:server:UpgradePlayerApartmentTier", function(tier, price, zkeaPrice)
    ...
    local apartment = Properties[propertyId]:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("UpgradeApartmentTier %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end

    if player.Functions.RemoveMoney("money", price) then
        local removed = exports["op-inventory"]:RemoveItem("cabinet_storage", "cabinet_zkea", zkeaPrice)
        if removed ~= false then
            MySQL.update.await("UPDATE housing_apartment SET tier = ? WHERE id = ?", {tier, apartmentId})

            exports["op-inventory"]:SetHouseStashMaxWeightFromTier(apartment:GetIdentifier(), tier)
...
RegisterNetEvent("housing:server:SetPlayerApartmentParkingPlace", function(hasParking, price)
    ...
    if property == nil then
        exports["op-monitor"]:Log("ERROR", ("SetApartmentParkingPlace %s - Apartment %s | skipped because it has no property"):format(propertyId, apartmentId))
        return
    end
    if not property:IsTrailer() then
        exports["op-monitor"]:Log("ERROR", ("SetApartmentParkingPlace %s - Apartment %s | skipped because it is not a trailer"):format(propertyId, apartmentId))
        return
    end
    local apartment = property:GetApartment(apartmentId)
    if apartment == nil then
        exports["op-monitor"]:Log("ERROR", ("SetApartmentParkingPlace %s - Apartment %s | skipped because it has no apartment"):format(propertyId, apartmentId))
        return
    end
