QBCore.Functions.CreateCallback("op-character:server:GetUserAccount", function(source, cb)
    local account = QBCore.Functions.GetUserAccount(source)

    if not account then
        if not GetConvar("op_allow_anonymous_login", "false") == "true" then
...
QBCore.Functions.CreateCallback("op-character:server:CreatePlayer", function(source, cb, charinfo, skin, clothConfig)
    ...
    if QBCore.Player.Login(source, false, player) then
        ...
        for _, item in pairs(Config.NewPlayerDefaultItems) do
            exports["op-inventory"]:AddItem(source, item.name, item.quantity, false, false)
        end
...
RegisterNetEvent("op-character:server:set-skin", function(skin, clothConfig)
...
RegisterNetEvent("op-character:server:InCharacterMenu", function(inMenu)
    if inMenu then
