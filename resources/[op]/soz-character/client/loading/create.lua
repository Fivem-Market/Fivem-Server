function CreatePlayer(shutdownLoadingScreen)
    ...
    if shutdownLoadingScreen then
        exports["op-loadscreen"]:Shutdown()
    end
    ...
    -- set God mode
    TriggerServerEvent("op-character:server:InCharacterMenu", true)
end
...
function CharacterPrepare()
    local account = QBCore.Functions.TriggerRpc("op-character:server:GetUserAccount")
    local charInfo = {}
...
function CharacterCreate(SpawnId, charInfo, character)
    ...
    local connected = QBCore.Functions.TriggerRpc("op-character:server:CreatePlayer", charInfo, character.Skin, character.ClothConfig);
