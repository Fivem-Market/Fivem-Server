...
RegisterNetEvent("op-personal-menu:cleanProps", function()
    cleanProps()
end)
    ...
    menu:AddButton({
        ...
        select = function()
            if favoriteAnimationRegister then
                SetResourceKvp("op/animation/" .. favoriteAnimationKey, json.encode({
                    label = content[1],
                    anim = content[5],
                }))
...
GenerateWalksList = function(menu, category, content)
    ...
    menu:AddButton({
        label = content.name,
        select = function()
            TriggerServerEvent("op-core:server:player:set-current-walkstyle", content.walk)
        end,
    })
...
function AnimationsEntry()
    ...
    for shortcut = 1, 10 do
        ...
        local animation = GetResourceKvpString("op/animation/" .. shortcut)
        personalAnimationMenu:AddSlider({
            ...
            select = function(_, value)
                ...
                elseif value == "delete" then
                    DeleteResourceKvp("op/animation/" .. shortcut)
                    MenuV:CloseAll(function()
                        personalAnimationMenu:Close()
                        animationMenu:Close()
                    end)
...
CreateThread(function()
    ...
    allWalksMenu:AddButton({
        ...
        select = function()
            TriggerServerEvent("op-core:server:player:set-current-walkstyle", "")
        end,
    })
    ...
    for shortcut = 1, 10 do
        ...
        RegisterCommand("animation_shortcut_" .. shortcut, function(source, args, rawCommand)
            if IsPedOnFoot(PlayerPedId()) then
                local kvpValue = GetResourceKvpString("op/animation/" .. shortcut)
                if kvpValue then
...
RegisterNetEvent("op-personal-menu:client:animation:surrender", function()
