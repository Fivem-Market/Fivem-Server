    ...
    return {
        {
            ...
            event = "op-jobs:client:baun:harvest",
            blackoutGlobal = true,
            blackoutJob = "baun",
            job = "baun",
...
RegisterNetEvent("op-jobs:client:baun:harvest", function(data)
    QBCore.Functions.TriggerCallback("op-jobs:server:baun:can-harvest", function(canHarvest, reason)
        if canHarvest then
            harvest(data)
        else
...
function harvest(data)
    ...
    exports["op-hud"]:DrawNotification(action_message)
    QBCore.Functions.Progressbar("harvest-crate", "Récolte en cours...", BaunConfig.Durations.Harvesting, false, true,
        ...
        QBCore.Functions.TriggerCallback("op-jobs:server:baun:harvest", function(success, reason)
            if success then
                TriggerEvent("op-jobs:client:baun:harvest", data)
            else
                if reason ~= "invalid_weight" then
                    exports["op-hud"]:DrawNotification(string.format("Il y a eu une erreur : `%s`", reason), "error")
                else
