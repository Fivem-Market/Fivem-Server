...
AddEventHandler("jobs:adsl:begin", function()
    ...
    TriggerServerEvent("job:set:pole", opJobCore.JobType.Adsl)
    OnJob = true
end)
...
AddEventHandler("jobs:adsl:tenue", function()
    QBCore.Functions.Progressbar("switch_clothes", "Changement d'habits...", 5000, false, true, {
        ...
        TriggerServerEvent("op-character:server:SetPlayerJobClothes", opJobCore.adsl_clothes[PlayerData.skin.Model.Hash])
...
AddEventHandler("jobs:adsl:vehicle", function()
    ...
    TriggerServerEvent("op-core:server:vehicle:free-job-spawn", "utillitruck3",
                       {
        opJobCore.adsl_vehicule.x,
        opJobCore.adsl_vehicule.y,
        opJobCore.adsl_vehicule.z,
        opJobCore.adsl_vehicule.w,
...
AddEventHandler("jobs:adsl:vehicle-spawn", function(vehicleNetId)
    ...
    JobVehicle = true
    createblip("Véhicule", "Montez dans le véhicule", 225, opJobCore.adsl_vehicule)
...
local function random_coord()
    local result = opJobCore.adsl[math.random(#opJobCore.adsl)]
...
local function close()
    ...
    local money = opJobCore.adsl_payout * payout_counter
