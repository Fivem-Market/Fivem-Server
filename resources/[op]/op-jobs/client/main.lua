...
CreateThread(function()
    while true do
        ...
        if NetworkIsSessionStarted() then
            TriggerServerEvent("op-jobs:AskJobSync")
            return
        end
    end
end)
...
RegisterNetEvent("op-jobs:Client:OnJobSync", function(jobs)
    opJobCore.Jobs = jobs
end)
...
exports("GetJobLabel", function(jobId)
    return opJobCore.Jobs[jobId].label
end)
...
local function BuildPromoteMenu(target)
    ...
    local job = opJobCore.Jobs[PlayerData.job.id]
    if job == nil then
...
CreateThread(function()
        ...
        options = {
            {
                ...
                canInteract = function(entity)
                    if not opJobCore.Functions.HasPermission(PlayerData.job.id, opJobCore.JobPermission.ManageGrade) and
                        not opJobCore.Functions.HasPermission(PlayerData.job.id, opJobCore.JobPermission.Enrollment) then
                        return false
                    end
                    ...
                    for _, bossZone in pairs(bossZones[PlayerData.job.id] or {}) do
                        if bossZone ~= nil and (bossZone:isPointInside(playerPosition) and bossZone:isPointInside(targetPosition)) then
                            local targetSource = GetPlayerServerId(NetworkGetPlayerIndexFromPed(entity));
                            local targetJob = QBCore.Functions.TriggerRpc("op-jobs:GetPlayerJob", targetSource)

                            return targetJob.id == opJobCore.JobType.Unemployed
                        end
                    end
    ...
    for jobId, job in pairs(opJobCore.Jobs) do
            ...
            exports["qb-target"]:AddGlobalPlayer({
                options = {
                    ...
                    },
                    {
                        ...
                        canInteract = function(entity)
                            if not PlayerData.job.onduty or not opJobCore.Functions.HasPermission(jobId, opJobCore.JobPermission.SocietyBankInvoices) then
                                return false
                            end
                            ...
                            return opJobCore.Jobs[targetJob.id].canReceiveSocietyInvoice
                        end,
                        job = jobId,
                        blackoutGlobal = true,
                        blackoutJob = jobId,
