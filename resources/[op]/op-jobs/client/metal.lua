...
exports["qb-target"]:AddBoxZone("job metal", vector3(-343.2, -1554.44, 25.23), 1, 1,
    ...
    options = {
        ...
        },
        {
            ...
            label = "S'équiper",
            job = opJobCore.JobType.Scrapper,
            blackoutGlobal = true,
...
AddEventHandler("jobs:metal:vente", function()
    ...
    local sellamount = exports["op-hud"]:Input(("Combien de ferrailles voulez-vous vendre ?"), 2, count)
...
AddEventHandler("jobs:metal:begin", function()
    ...
    TriggerServerEvent("job:set:pole", opJobCore.JobType.Scrapper)
    OnJob = true
end)
...
AddEventHandler("jobs:metal:tenue", function()
    QBCore.Functions.Progressbar("switch_clothes", "Changement d'habits...", 5000, false, true, {
        ...
        TriggerServerEvent("op-character:server:SetPlayerJobClothes", opJobCore.metal_clothes[PlayerData.skin.Model.Hash])
...
AddEventHandler("jobs:metal:vehicle", function()
    ...
    TriggerServerEvent("op-core:server:vehicle:free-job-spawn", "scrap",
                       {
        opJobCore.metal_vehicule.x,
        opJobCore.metal_vehicule.y,
        opJobCore.metal_vehicule.z,
        opJobCore.metal_vehicule.w,
...
AddEventHandler("jobs:metal:vehicle-spawn", function(vehicleNetId)
    ...
    JobVehicle = true
    createblip("Véhicule", "Montez dans le véhicule", 225, opJobCore.metal_vehicule)
...
local function random_coord()
    local result = opJobCore.metal[math.random(#opJobCore.metal)]
    while result.x == JobDone do
        result = opJobCore.metal[math.random(#opJobCore.metal)]
    end
    JobDone = result.x
    return result
end
