QBCore = exports["qb-core"]:GetCoreObject()

exports("GetCoreObject", function()
    return opJobCore
end)

MySQL.ready(function()
    SynchroniseJob()
end)

function SynchroniseJob()
    local jobGrades = MySQL.query.await("SELECT * FROM job_grades", {})
    local tmpGrades = {}

    for _, jobId in pairs(opJobCore.JobType) do
        tmpGrades[jobId] = {}
    end

    if jobGrades then
        for _, jobGrade in ipairs(jobGrades) do
            if not opJobCore.Jobs[jobGrade.jobId] then
                exports["op-monitor"]:Log("ERROR", ("Job %s (grade %s) is not present in opJobCore !"):format(jobGrade.jobId, jobGrade.name))
                goto continue
            end

            jobGrade.permissions = json.decode(jobGrade.permissions)
            tmpGrades[jobGrade.jobId][tostring(jobGrade.id)] = jobGrade

            ::continue::
        end
    end

    for _, jobId in pairs(opJobCore.JobType) do
        opJobCore.Jobs[jobId].grades = tmpGrades[jobId]
    end

    TriggerClientEvent("op-jobs:Client:OnJobSync", -1, opJobCore.Jobs)
end

function CheckPlayerJobPermission(player, targetJobId, permission)
    if not player then
        return false
    end

    return CheckJobPermission(targetJobId, player.job.id, player.job.grade, permission)
end

function GetJobDefaultGrade(jobId)
    local gradeId = 0
    local job = opJobCore.Jobs[jobId]

    if job == nil then
        return gradeId
    end

    for id, grade in pairs(job.grades) do
        if grade.is_default == 1 then
            gradeId = id

            break
        end
    end

    return gradeId
end

function CheckJobPermission(targetJobId, jobId, gradeId, permission)
    if targetJobId ~= jobId then
        return false
    end

    if not opJobCore.Jobs[jobId] then
        return false
    end

    local job = opJobCore.Jobs[jobId]

    if not job.grades[tostring(gradeId)] then
        return false
    end

    local grade = job.grades[tostring(gradeId)]

    if grade.owner == 1 then
        return true
    end

    local permissions = grade.permissions

    if permissions ~= nil then
        for _, v in pairs(permissions) do
            if v == permission then
                return true
            end
        end
    end

    return false
end

QBCore.Functions.CreateCallback("op-jobs:GetPlayerJob", function(source, cb, target)
    local targetPlayer = QBCore.Functions.GetPlayer(tonumber(target))

    cb(targetPlayer.PlayerData.job)
end)

RegisterServerEvent("op-jobs:AskJobSync", function()
    TriggerClientEvent("op-jobs:Client:OnJobSync", source, opJobCore.Jobs)
end)
