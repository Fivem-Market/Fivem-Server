...
RegisterNetEvent("job:server:placeProps", function(item, props, rotation, offset)
    ...
    if exports["op-inventory"]:GetItem(Player.PlayerData.source, item, nil, true) >= 1 then
        exports["op-inventory"]:RemoveItem(Player.PlayerData.source, item, 1)
...
RegisterNetEvent("job:server:AddObject", function(object, position)
    exports["op-utils"]:CreateObject(object, position.x, position.y, position.z, position.w or 0, 8000.0, ObjectWithoutFreeze[object] ~= true)
end)

RegisterNetEvent("job:server:RemoveObject", function(ref)
    exports["op-utils"]:DeleteObject(ref)
end)
...
RegisterNetEvent("job:server:CollectObject", function(ref, model)
    ...
    if item then
        if exports["op-inventory"]:CanCarryItem(Player.PlayerData.source, item, 1) then
            exports["op-inventory"]:AddItem(Player.PlayerData.source, item, 1)
