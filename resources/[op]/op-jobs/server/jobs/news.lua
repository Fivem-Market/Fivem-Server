...
QBCore.Functions.CreateUseableItem("newspaper", function(source, item)
    ...
    if Player.PlayerData.job.id ~= opJobCore.JobType.News then
        return
    end
...
RegisterNetEvent("jobs:server:news:newspaperSold", function()
    ...
    local playerNewspaper = exports["op-inventory"]:GetItem(Player.PlayerData.source, "newspaper", nil, true)
    ...
    exports["op-inventory"]:RemoveItem(Player.PlayerData.source, "newspaper", newspaperAmount)
...
RegisterNetEvent("jobs:server:news:newspaperFarm", function()
    ...
    exports["op-inventory"]:AddItem(Player.PlayerData.source, "newspaper", newspaperAmount, nil, nil, function(success)
        if success then
...
RegisterNetEvent("jobs:server:news:UseMobileItem", function(item, event)
    ...
    if (exports["op-inventory"]:GetItem(Player.PlayerData.source, item, nil, true) or 0) >= 1 then
