QBCore.Functions.CreateCallback("op-jobs:server:baun:can-craft", function(source, cb, itemId)
    ...
    for _, ingredient in pairs(ingredients) do
        ...
        local item = exports["op-inventory"]:GetItem(source, ingredient.itemId, nil)
        if item.amount < ingredient.quantity or exports["op-utils"]:ItemIsExpired(item) then
...
QBCore.Functions.CreateCallback("op-jobs:server:baun:craft", function(source, cb, itemId)
    local player = QBCore.Functions.GetPlayer(source)
    if player == nil then
        cb(false, "Le joueur n'existe pas.")
        return
    ...
    exports["op-inventory"]:AddItem(source, itemId, 1, nil, nil, function(success, reason)
        if not success then
...
QBCore.Functions.CreateCallback("op-jobs:server:baun:can-harvest", function(source, cb, item)
    if exports["op-inventory"]:CanCarryItem(source, item, 1) then
        cb(true, nil)
    else
...
QBCore.Functions.CreateCallback("op-jobs:server:baun:harvest", function(source, cb, itemId)
    local player = QBCore.Functions.GetPlayer(source)
    if player == nil then
        cb(false, "Le joueur n'existe pas.")
        return
...
QBCore.Functions.CreateCallback("op-jobs:server:baun:can-restock", function(source, cb, itemId)
    local canRestock = exports["op-inventory"]:GetItem(source, itemId, nil, true) > 0
    cb(canRestock)
end)

QBCore.Functions.CreateCallback("op-jobs:server:baun:restock", function(source, cb, storage, itemId)
    ...
    if not cbCalled and exports["op-inventory"]:GetItem(source, itemId, nil, true) == 0 then
        cbCalled = true
        cb(false, "missing_ingredient")
        return
    end
    ...
    for _, item in pairs(BaunConfig.Restock[itemId]) do
        if not cbCalled then
            exports["op-inventory"]:AddItem(storage, item.itemId, item.quantity, nil, nil, function(success, reason)
...
QBCore.Functions.CreateCallback("op-jobs:server:baun:createCocktailBox", function(source, cb)
    local player = QBCore.Functions.GetPlayer(source)
    local playerCocktails = {}
    local cocktailsToRemove = 10
    for _, item in pairs(player.PlayerData.items) do
        if item.type == "cocktail" and item.amount > 0 and not exports["op-utils"]:ItemIsExpired(item) then
    ...
    if not exports["op-inventory"]:CanCarryItems(source, checkList) then
    ...
    exports["op-inventory"]:AddItem(source, "cocktail_box", 1, nil, nil, function(success, reason)
        if not success then
            TriggerClientEvent("hud:client:DrawNotification", source, "Vos poches sont pleines...", "error")
        else
