...
QBCore.Functions.CreateCallback("op-jobs:server:fueler:getFieldHealth", function(source, cb, fieldName)
...
QBCore.Functions.CreateCallback("jobs:server:fueler:refillTanker", function(source, cb, tankerId, fieldName)
    ...
    exports["op-inventory"]:AddItem("trunk_" .. tankerPlate, item, quantity, nil, nil, function(success, _)
        if success then
...
RegisterNetEvent("jobs:server:fueler:refiningTanker", function(tankerId)
    ...
    if exports["op-inventory"]:RemoveItem(tankerInv, "petroleum", itemToRefill) then
        local refinedSuccess = false

        exports["op-inventory"]:AddItem(tankerInv, "petroleum_refined", 3 * itemToRefill, nil, nil, function(success, _)
            refinedSuccess = refinedSuccess or success
        end)
        exports["op-inventory"]:AddItem(tankerInv, "petroleum_residue", itemToRefill, nil, nil, function(success, _)
            refinedSuccess = refinedSuccess or success
        end)
...
RegisterNetEvent("jobs:server:fueler:resetTanker", function(tankerId)
    ...
    if GetVehicleType(tanker) == "trailer" then
        exports["op-inventory"]:Clear(tankerInv, false)
    end
end)

RegisterNetEvent("jobs:server:fueler:craftEssence", function()
    local Player = QBCore.Functions.GetPlayer(source)
    local essenceItemAmount = exports["op-inventory"]:GetItem(Player.PlayerData.source, "petroleum_refined", nil, true)

    if exports["op-inventory"]:CanSwapItem(Player.PlayerData.source, "petroleum_refined", essenceItemAmount, "essence", essenceItemAmount) then
        if exports["op-inventory"]:RemoveItem(Player.PlayerData.source, "petroleum_refined", essenceItemAmount) then
            exports["op-inventory"]:AddItem(Player.PlayerData.source, "essence", essenceItemAmount, nil, nil, function(success, _)
                if success then
...
RegisterNetEvent("jobs:server:fueler:craftEssenceJerryCan", function()
    local Player = QBCore.Functions.GetPlayer(source)
    local essenceItemAmount = math.floor(exports["op-inventory"]:GetItem(Player.PlayerData.source, "essence", nil, true) / 3)

    if exports["op-inventory"]:CanSwapItem(Player.PlayerData.source, "essence", essenceItemAmount * 3, "essence_jerrycan", essenceItemAmount) then
        if exports["op-inventory"]:RemoveItem(Player.PlayerData.source, "essence", essenceItemAmount * 3) then
            exports["op-inventory"]:AddItem(Player.PlayerData.source, "essence_jerrycan", essenceItemAmount, nil, nil, function(success, _)
                if success then
                    TriggerClientEvent("hud:client:DrawNotification", Player.PlayerData.source,
...
RegisterNetEvent("jobs:server:fueler:craftKerosene", function()
    local Player = QBCore.Functions.GetPlayer(source)
    local keroseneItemAmount = math.floor(exports["op-inventory"]:GetItem(Player.PlayerData.source, "petroleum_refined", nil, true) / 4)
    ...
    if not exports["op-inventory"]:CanSwapItem(Player.PlayerData.source, "petroleum_refined", petrolItemAmount, "kerosene", keroseneItemAmount) then
        TriggerClientEvent("hud:client:DrawNotification", Player.PlayerData.source, "Votre ne pouvez pas ~r~transformer~s~ autant d'essence.", "error")
        return
    end

    if exports["op-inventory"]:RemoveItem(Player.PlayerData.source, "petroleum_refined", petrolItemAmount) then
        exports["op-inventory"]:AddItem(Player.PlayerData.source, "kerosene", keroseneItemAmount, nil, nil, function(success, _)
            if success then
                TriggerClientEvent("hud:client:DrawNotification", Player.PlayerData.source,
...
RegisterNetEvent("jobs:server:fueler:craftKeroseneJerryCan", function()
    local Player = QBCore.Functions.GetPlayer(source)
    local jerrycanItemAmount = math.floor(exports["op-inventory"]:GetItem(Player.PlayerData.source, "kerosene", nil, true) / 3)
    local keroseneItemAmount = jerrycanItemAmount * 3

    if not exports["op-inventory"]:CanSwapItem(Player.PlayerData.source, "kerosene", keroseneItemAmount, "kerosene_jerrycan", jerrycanItemAmount) then
    ...
    if exports["op-inventory"]:RemoveItem(Player.PlayerData.source, "kerosene", keroseneItemAmount) then
        exports["op-inventory"]:AddItem(Player.PlayerData.source, "kerosene_jerrycan", jerrycanItemAmount, nil, nil, function(success, _)
            if success then
                TriggerClientEvent("hud:client:DrawNotification", Player.PlayerData.source,
...
QBCore.Functions.CreateCallback("jobs:server:fueler:resellTanker", function(source, cb, tankerId)
    ...
    local essenceItemAmount = exports["op-inventory"]:GetItem("trunk_" .. tankerPlate, "essence", nil, true)
    local keroseneItemAmount = exports["op-inventory"]:GetItem("trunk_" .. tankerPlate, "kerosene", nil, true)

    if essenceItemAmount >= 10 then
        if exports["op-inventory"]:RemoveItem(tankerInv, "essence", 10) then
    ...
    if keroseneItemAmount >= 10 then
        if exports["op-inventory"]:RemoveItem(tankerInv, "kerosene", 10) then
            TriggerEvent("banking:server:TransferMoney", "farm_mtp", "safe_oil", 10 * FuelerConfig.SellPrice)
...
QBCore.Functions.CreateCallback("jobs:server:fueler:ensureInventory", function(source, cb, tankerId, model, class)
    local tanker = NetworkGetEntityFromNetworkId(tankerId)
    local tankerPlate = GetVehicleNumberPlateText(tanker)
    local inventory = exports["op-inventory"]:GetOrCreateInventory("tanker", tankerPlate, {
        model = model,
        class = class,
    })
...
QBCore.Functions.CreateCallback("jobs:server:fueler:canRefill", function(source, cb, tankerId)
    ...
    cb(exports["op-inventory"]:CanCarryItem("trunk_" .. tankerPlate, "petroleum", itemToRefill))
end)
...
QBCore.Functions.CreateCallback("jobs:server:fueler:canRefining", function(source, cb, tankerId)
    ...
    cb(exports["op-inventory"]:CanSwapItem("trunk_" .. tankerPlate, srcItem, srcItemAmount, dstItem, dstItemAmount))
end)
...
QBCore.Functions.CreateCallback("jobs:server:fueler:canStationRefill", function(source, cb, tankerId, amount)
    ...
    cb(exports["op-inventory"]:GetItem("trunk_" .. tankerPlate, "essence", nil, true) >= itemToUse)
end)
...
QBCore.Functions.CreateCallback("jobs:server:fueler:canKeroseneStationRefill", function(source, cb, amount)
    ...
    cb(exports["op-inventory"]:GetItem(Player.PlayerData.source, "kerosene", nil, true) >= itemToUse)
end)
