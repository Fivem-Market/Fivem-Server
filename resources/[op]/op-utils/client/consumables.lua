...
RegisterNetEvent("QBCore:Player:SetPlayerData", function(PlayerData)
    ...
    if PlayerData.metadata["alcohol"] > 80 or PlayerData.metadata["drug"] > 80 then
        ...
        TriggerEvent("op-core:client:player:update-walk-style", "move_m@drunk@verydrunk")
    elseif PlayerData.metadata["alcohol"] > 40 or PlayerData.metadata["drug"] > 40 then
        ShakeGameplayCam("DRUNK_SHAKE", 0.5)
        SetPedIsDrunk(PlayerPedId(), true)
        TriggerEvent("op-core:client:player:update-walk-style", "move_m@drunk@moderatedrunk")
    elseif PlayerData.metadata["alcohol"] > 0 or PlayerData.metadata["drug"] > 0 then
        ShakeGameplayCam("DRUNK_SHAKE", 0.0)
        SetPedIsDrunk(PlayerPedId(), false)
        TriggerEvent("op-core:client:player:update-walk-style", "move_m@drunk@slightlydrunk")
    else
        ShakeGameplayCam("DRUNK_SHAKE", 0.0)
        SetPedIsDrunk(PlayerPedId(), false)
    end
...
RegisterNetEvent("scuba:client:Toggle", function(scuba)
    ...
    if scuba then
        QBCore.Functions.Progressbar("switch_clothes", "Changement d'habits...", 5000, false, true, {
            ...
            TriggerServerEvent("op-character:server:SetPlayerJobClothes", skin[PlayerData.skin.Model.Hash], false)
        end)
    else
        QBCore.Functions.Progressbar("switch_clothes", "Changement d'habits...", 5000, false, true, {
            disableMovement = true,
            ...
            TriggerServerEvent("op-character:server:SetPlayerJobClothes", nil, false)
        end)
    end
end)
