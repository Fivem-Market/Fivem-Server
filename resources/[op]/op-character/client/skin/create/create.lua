local createCharacterMenu = MenuV:CreateMenu(nil, "", "menu_char_creation", "op", "create-character")
...
function CreateCharacterWizard(spawnId, character)
    ...
    while not confirm do
        ...
        local confirmWord = exports["op-hud"]:Input("Entrer 'OUI' pour confirmer le skin de ce personnage", 32) or ""

        if confirmWord:lower() == "oui" then
            confirm = true
            TriggerServerEvent("op-character:server:InCharacterMenu", false)
        end
    end
...
RegisterNetEvent("op-character:client:RequestCharacterWizard", function()
    ...
    TriggerServerEvent("op-character:server:set-skin", character.Skin, character.ClothConfig)
    exports["op-hud"]:DrawNotification("Votre chirurgie s'est bien passée !")
end)
