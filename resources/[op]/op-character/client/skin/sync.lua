...
RegisterNetEvent("op-character:Client:ApplyTemporaryClothSet", function(clothSet)
...
RegisterNetEvent("op-character:Client:ApplyTemporarySkin", function(skin)
    ApplyPlayerBodySkin(PlayerId(), skin)
end)

-- Apply la configuration de vetements du joueur (données persisté)
RegisterNetEvent("op-character:Client:ApplyCurrentClothConfig", function()
    ApplyPlayerClothConfig(PlayerId(), PlayerData.cloth_config)
end)

-- Apply le skin du joueur (données persisté)
RegisterNetEvent("op-character:Client:ApplyCurrentSkin", function()
...
RegisterNetEvent("op-character:Client:SetTemporaryNaked", function()
    local tempClothConfig = Clone(PlayerData.cloth_config)
