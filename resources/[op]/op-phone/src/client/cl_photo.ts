...
const takePhoto = () =>
    new Promise((res, rej) => {
        exp['screenshot-basic'].requestScreenshotUpload(
            `${GetConvar('op_public_api_endpoint', 'https://api.op.zerator.com')}/graphql`,
            'GQL',
            {
                encoding: config.images.imageEncoding,
                headers: {
                    authorization: `Bearer ${LocalPlayer.state.opJWTToken}`,
                },
            },
            async (data: any) => {
                try {
                    ...
                    const resp = await ClUtils.emitNetPromise(
                        ...
                        GetConvar('op_public_endpoint', 'https://op.zerator.com') + parsedData
                    );
                    res(resp);
                } catch (e) {
                    rej(e.toString());
