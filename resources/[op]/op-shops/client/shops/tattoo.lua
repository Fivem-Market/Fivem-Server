...
function TattooShop:GenerateMenu(skipIntro)
    ...
    shopMenu:AddButton({
        ...
        select = function(item)
            local validation = exports["op-hud"]:Input("Voulez-vous vraiment ce tatouage ? [oui/non]", 3)
...
function TattooShop:PreGenerateMenu(skipIntro)
    ...
    -- set naked
    TriggerEvent("op-character:Client:SetTemporaryNaked", true)
...
function TattooShop:OnMenuClose()
    ...
    -- Unset naked
    TriggerEvent("op-character:Client:ApplyCurrentClothConfig")
    TriggerEvent("op-character:Client:ApplyCurrentSkin")
end
...
function TattooShop:MenuEntryAction(item)
    ...
    TriggerEvent("op-character:Client:ApplyTemporarySkin", skin)
end
