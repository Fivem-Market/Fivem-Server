...
RegisterNetEvent("pawl:client:harvestTree", function(data)
    ...
    exports["op-hud"]:DrawNotification("Vous êtes en train de ~g~couper l’arbre~s~.")
    local success, _ = exports["op-utils"]:Progressbar("harvest-tree", "Vous récoltez...", Config.Harvest.Duration, false, true,
    ...
    if success then
        ...
        if cutTree then
            ...
            exports["op-hud"]:DrawNotification("Vous avez ~g~découpé~s~ l’arbre.")
        else
            exports["op-hud"]:DrawNotification("Vous avez ~r~raté~s~ la découpe de l’arbre.")
        end
    end
...
RegisterNetEvent("pawl:client:harvestTreeSap", function(data)
    ...
    exports["op-hud"]:DrawNotification("Vous êtes en train de ~g~récolter de la sève~s~.")
    local success, _ = exports["op-utils"]:Progressbar("harvest-tree", "Vous récoltez...", Config.Harvest.SapDuration, false, true,
                                                        {disableMovement = true, disableCombat = true},
                                                        {
        animDict = "melee@large_wpn@streamed_core",
        anim = "plyr_rear_takedown_bat_r_facehit",
    ...
    if success then
        ...
        if sapTree then
            ...
            exports["op-hud"]:DrawNotification("Vous avez ~g~récolté~s~ de la sève.")
        else
            exports["op-hud"]:DrawNotification("Vous avez ~r~raté~s~ la récolte de la sève.")
        end
    end
...
RegisterNetEvent("pawl:client:syncField", function(identifier, data)
    ...
    local currentTime = exports["op-utils"]:GetTimestamp() / 1000
