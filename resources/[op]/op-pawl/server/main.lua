...
QBCore.Functions.CreateCallback("pawl:server:harvestTree", function(source, cb, identifier, position)
    ...
    if exports["op-inventory"]:CanCarryItems(Player.PlayerData.source, Config.Harvest.RewardItems) then
        ...
        if harvest then
            ...
            for _, item in pairs(Config.Harvest.RewardItems) do
                ...
                exports["op-inventory"]:AddItem(Player.PlayerData.source, item.name, item.amount, nil, nil, function(success, reason)
                    if not success then
                        cb(false)
                        cbSent = true
                    end
...
QBCore.Functions.CreateCallback("pawl:server:harvestTreeSap", function(source, cb, identifier, position)
    ...
    if exports["op-inventory"]:CanCarryItems(Player.PlayerData.source, Config.Harvest.SecondaryRewardItems) then
...
RegisterNetEvent("pawl:server:startProcessingTree", function(data)
    ...
    if not exports["op-inventory"]:CanCarryItem(Config.Processing.PlankStorage, Config.Processing.PlankItem, Config.Processing.PlankAmount) then
        TriggerClientEvent("hud:client:DrawNotification", Player.PlayerData.source, "Le stockage de planches est plein !", "error")
        return
    end

    if not exports["op-inventory"]:CanCarryItem(Config.Processing.SawdustStorage, Config.Processing.SawdustItem, Config.Processing.SawdustAmount) then
    ...
    Citizen.CreateThread(function()
        while Processing.Enabled do
            if GetGameTimer() - Processing.StartedAt >= Config.Processing.Duration then
                if exports["op-inventory"]:RemoveItem(Config.Processing.ProcessingStorage, Config.Processing.ProcessingItem, Config.Processing.ProcessingAmount) then
                    exports["op-inventory"]:AddItem(Config.Processing.PlankStorage, Config.Processing.PlankItem, Config.Processing.PlankAmount)
                    exports["op-inventory"]:AddItem(Config.Processing.SawdustStorage, Config.Processing.SawdustItem, Config.Processing.SawdustAmount)

                    if exports["op-inventory"]:GetItem(Config.Processing.ProcessingStorage, Config.Processing.ProcessingItem, nil, true) >= 1 then
                        Processing.StartedAt = GetGameTimer()
                    else
...
RegisterNetEvent("pawl:server:craft", function(identifier)
    ...
    if not exports["op-inventory"]:CanCarryItem(Player.PlayerData.source, craft.RewardItem, craft.RewardAmount) then
    ...
    local craftItemInventory = exports["op-inventory"]:GetItem(Player.PlayerData.source, craft.SourceItem, nil, true)
    ...
    if exports["op-inventory"]:RemoveItem(Player.PlayerData.source, craft.SourceItem, craft.SourceItemAmount or 1) then
        exports["op-inventory"]:AddItem(Player.PlayerData.source, craft.RewardItem, craft.RewardAmount, metadata, nil, function(success, reason)
            if success then
