...
RegisterNetEvent("police:server:CuffPlayer", function(targetId, isSoftcuff)
    ...
    if target then
        if player.Functions.GetItemByName("handcuffs") then
            exports["op-inventory"]:RemoveItem(player.PlayerData.source, "handcuffs", 1)
            ...
            TriggerClientEvent("op-talk:client:PowerOffRadio", target.PlayerData.source)
        else
...
RegisterNetEvent("police:server:UnCuffPlayer", function(targetId)
    ...
    if target then
        if player.Functions.GetItemByName("handcuffs_key") then
            exports["op-inventory"]:RemoveItem(player.PlayerData.source, "handcuffs_key", 1)
