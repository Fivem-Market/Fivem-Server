...
RegisterNuiCB<void>(EmergencyEvents.LSMC_CALL, async (_, cb) => {
    TriggerEvent('op-core:lsmc:client:call');
    cb({});
});

RegisterNuiCB<void>(EmergencyEvents.UHU_CALL, async (_, cb) => {
    TriggerServerEvent('op-core:lsmc:server:revive', null, true, true);
    hidePhone();
    cb({});
});
