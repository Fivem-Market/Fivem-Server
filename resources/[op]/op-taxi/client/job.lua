            ...
            if dist < 5 then
                if IsVehicleStopped(GetVehiclePedIsIn(ped, 0)) and ValidVehicle() then
                    ...
                    if not IsPedInVehicle(NpcData.Npc, veh, false) then
                        ClearNpcMission()
                        exports["op-hud"]:DrawNotification("Vous n'avez pas la personne dans votre véhicule")
                        return
                    end
                    ...
                    TotalDistance = 0
                    exports["op-hud"]:DrawNotification("Vous avez déposé la personne")
                    if NpcData.DeliveryBlip ~= nil then
                        RemoveBlip(NpcData.DeliveryBlip)
                    end
                    local RemovePed = function(ped)
...
RegisterNetEvent("taxi:client:DoTaxiNpc", function()
    if ValidVehicle() then
        if not NpcData.Active then
                        ...
                        if IsVehicleStopped(GetVehiclePedIsIn(ped, 0)) and ValidVehicle() then
                            ...
                            while not IsPedInVehicle(NpcData.Npc, veh, false) do
                                ...
                                if count == 15 or dist > requiredDist then
                                    ClearNpcMission()
                                    exports["op-hud"]:DrawNotification("Ouvre ton véhicule la prochaine fois ?")
                                    return
                                end
                                Wait(1000)
                                TaskEnterVehicle(NpcData.Npc, veh, -1, freeSeat, 1.0, 0)
                            ...
                            exports["op-hud"]:DrawNotification("Amenez la personne à la destination spécifiée")
                            if NpcData.NpcBlip ~= nil then
                                RemoveBlip(NpcData.NpcBlip)
                            end
                            GetDeliveryLocation()
            ...
            exports["op-hud"]:DrawNotification("Vous êtes déjà en mission")
        end
    else
        exports["op-hud"]:DrawNotification("Vous n'êtes pas dans un taxi")
    end
end)
