...
export class PlayerProvider {
    ...
    public async getJwtToken(source: number): Promise<string | null> {
        ...
        const url = GetConvar('op_api_endpoint', 'https://api.op.zerator.com') + '/accounts/create-token/' + steam;
        const response = await axios.get(url, {
            auth: {
                username: GetConvar('op_api_username', 'admin'),
                password: GetConvar('op_api_password', 'admin'),
            },
        });
