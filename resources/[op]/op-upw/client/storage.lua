...
function CreateInverterZone(identifier, data)
    exports["op-utils"]:CreateObjectClient(identifier, GetHashKey("upwpile"), data.coords.x, data.coords.y, data.coords.z, data.heading, 8000.0, true)
        ...
        if isIn then
            Citizen.CreateThread(function()
                while RunInverterRefreshLoop do
                    local capacity = QBCore.Functions.TriggerRpc("op-upw:server:GetInverterStorage", identifier)
...
local function getTerminalProp(scope)
    if scope == "default" then
        return "op_prop_elec01"
    elseif scope == "entreprise" then
        return "op_prop_elec02"
    end
end
...
function CreateTerminalZone(identifier, data, facility)
    ...
    local entity =
        exports["op-utils"]:CreateObjectClient(identifier, GetHashKey(prop), data.coords.x, data.coords.y, data.coords.z, data.heading, 8000.0, true)
    ...
    local options = {
        {
            ...
            action = function(entity)
                TriggerEvent("op-upw:client:HarvestLoop", {
        ...
        },
        {
            ...
            action = function(entity)
                TriggerServerEvent("op-upw:server:FacilityCapacity", {
                    identifier = entitiesToId[entity].identifier,
                    facility = "terminal",
                })
            end,
