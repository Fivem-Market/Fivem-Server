...
-- EVENTS
--
QBCore.Functions.CreateCallback("op-upw:server:GetPlantActive", function(source, cb, identifier)
...
RegisterNetEvent("op-upw:server:TogglePlantActive", function(data)
...
QBCore.Functions.CreateCallback("op-upw:server:PrecheckHarvest", function(source, cb, identifier, harvestType)
    ...
    if harvestType == "terminal-in" then
        local items = exports["op-inventory"]:GetItemsByType(Player.PlayerData.source, "energy")
        local amount = 0
        ...
        local canCarry = exports["op-inventory"]:CanCarryItem(Player.PlayerData.source, item, 1)
...
QBCore.Functions.CreateCallback("op-upw:server:Harvest", function(source, cb, identifier, harvestType)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if harvestType == "terminal-in" then
        ...
        local invChanged = exports["op-inventory"]:RemoveItem(Player.PlayerData.source, firstItem.item.name, 1)
        ...
        exports["op-inventory"]:AddItem(Player.PlayerData.source, item, count, nil, nil, function(success, reason)
...
QBCore.Functions.CreateCallback("op-upw:server:GetWaste", function(source, cb, identifier, harvestType)
    local facilityData = GetFacilityData(harvestType)
    local facility = facilityData.getFacility(identifier)
    if not facility then
        cb(false, "invalid facility")
