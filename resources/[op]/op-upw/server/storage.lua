...
RegisterNetEvent("op-upw:server:FacilityCapacity", function(data)
...
QBCore.Functions.CreateCallback("op-upw:server:GetInverterStorage", function(source, cb, identifier)
    local items = exports["op-inventory"]:GetItemsByType("inverter_" .. identifier, "energy")
    local weight = 0
...
QBCore.Functions.CreateCallback("op-upw:server:GetBlackoutLevel", function(source, cb)
    cb(GetBlackoutLevel())
end)

QBCore.Functions.CreateCallback("op-upw:server:IsJobBlackout", function(source, cb, jobId)
...
function StartConsumptionLoop()
    Citizen.CreateThread(function()
        ...
        while consumptionLoopRunning do
            ...
            for jobId, v in pairs(opJobCore.Jobs) do
...
RegisterNetEvent("op-upw:server:ConsumeTerminalRatio", function(id, value)
