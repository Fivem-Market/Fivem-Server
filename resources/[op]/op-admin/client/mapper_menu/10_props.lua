...
function BuildJobList()
    local opJobCore = exports["op-jobs"]:GetCoreObject()
    local JobList = {}

    for jobID, jobData in pairs(opJobCore.Jobs) do
...
local function selectModel(model)
    if model == nil then
        model = exports["op-hud"]:Input("Nom du modèle : ", 32)
    end
...
function MapperMenuProps(menu)
    ...
    propsMenu:AddSlider({
        ...
        value = nil,
        values = {
            {label = "poubelle", value = "op_prop_bb_bin"},
            {label = "borne civile", value = "op_prop_elec01___default"},
            {label = "borne entreprise", value = "op_prop_elec02___entreprise"},
            {label = "onduleur", value = "upwpile"},
            {label = "ATM entreprise", value = "op_atm_entreprise"},
        },
    ...
    propsMenu:AddButton({
        ...
        select = function()
            ...
            if PropOption.model == "op_prop_elec01" or PropOption.model == "op_prop_elec02" or PropOption.model == "upwpile" then
                TriggerServerEvent("op-upw:server:AddFacility", PropOption.model, PropOption.propCoord, PropOption.scope, PropOption.job)
            else
