...
houseMenu:On("open", function(menu)
    ...
    menu:AddButton({
        ...
        select = function()
            local identifier = exports["op-hud"]:Input("Nom du bâtiment:", 50)
            if identifier == nil or #identifier == 0 then
                exports["op-hud"]:DrawNotification("Le nom ne peut pas être vide", "error")
                return
            end
            QBCore.Functions.TriggerRpc("admin:server:housing:CreateProperty", identifier)
            exports["op-hud"]:DrawNotification("Bâtiment ajouté", "success")
            menu:Close()
            menu:Open()
        end,
    })
...
currentPropertyMenu:On("open", function(menu)
    ...
    menu:AddButton({
        ...
        select = function()
            local identifier = exports["op-hud"]:Input("Identifiant de l'intérieur :", 50)
            if identifier == nil or #identifier == 0 then
                exports["op-hud"]:DrawNotification("Le nom ne peut pas être vide", "error")
                return
            end

            local label = exports["op-hud"]:Input("Nom de l'intérieur :", 50)
            ...
            exports["op-hud"]:DrawNotification("Intérieur ajouté", "success")
            menu:Close()
            menu:Open()
        end,
    })
    ...
    menu:AddConfirm(({
        ...
        value = "n",
        confirm = function()
            local confirm = exports["ophud"]:Input("Êtes-vous sûr de vouloir supprimer la propriété ?", 5, "yes/no")
            if confirm == "yes" then
...
currentApartmentMenu:On("open", function(menu)
    ...
    menu:AddButton({
        ...
        select = function()
            local identifier = exports["op-hud"]:Input("Identifiant de l'intérieur:", 50, HouseOption.CurrentApartmentData.identifier)
            if identifier == nil or #identifier == 0 then
                exports["op-hud"]:DrawNotification("Le nom ne peut pas être vide", "error")
                return
            end
            ...
            QBCore.Functions.TriggerRpc("admin:server:housing:SetApartmentIdentifier", HouseOption.CurrentPropertyData.id, HouseOption.CurrentApartmentData.id,
                                        identifier)
            exports["op-hud"]:DrawNotification("Identifiant de l'intérieur modifié", "success")
    ...
    menu:AddButton({
        ...
        select = function()
            local label = exports["op-hud"]:Input("Nom de l'intérieur:", 50, HouseOption.CurrentApartmentData.label)
            if label == nil or #label == 0 then
                exports["op-hud"]:DrawNotification("Le nom ne peut pas être vide", "error")
                return
            end
            ...
            exports["op-hud"]:DrawNotification("Le nom a été modifié", "success")
    ...
    menu:AddButton({
        ...
        select = function()
            ...
            QBCore.Functions.TriggerRpc("admin:server:housing:SetApartmentInsideCoord", HouseOption.CurrentPropertyData.id, HouseOption.CurrentApartmentData.id,
                                        coord)
            exports["op-hud"]:DrawNotification("La position d'apparition a été modifiée", "success")
        end,
    })
    ...
    menu:AddSlider({
        ...
        select = function(_, value)
            if value == "custom" and PlayerData.role == "admin" then
                local price = exports["op-hud"]:Input("Prix de l'intérieur:", 50, HouseOption.CurrentApartmentData.price)
                if price == nil or #price == 0 then
                    exports["op-hud"]:DrawNotification("Le prix ne peut pas être vide", "error")
                    return
                end
                value = price
            else
                exports["op-hud"]:DrawNotification("Vous n'avez pas accès a cette option", "error")
                return
            end
            ...
            exports["op-hud"]:DrawNotification("Le prix a été modifié", "success")
    ...
    menu:AddConfirm(({
        ...
        value = "n",
        confirm = function()
            local confirm = exports["op-hud"]:Input("Êtes-vous sûr de vouloir supprimer l'intérieur ?", 5, "yes/no")
            if confirm == "yes" then
...
currentEditingMenu:On("open", function(menu)
    ...
    menu:AddButton({
        ...
        select = function()
            ...
            exports["op-hud"]:DrawNotification("La zone a été modifiée", "success")
            menu:Close()
        end,
    })
end)
