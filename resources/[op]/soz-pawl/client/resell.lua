...
Citizen.CreateThread(function()
    local resellOpt = opJobCore.Jobs[opJobCore.JobType.Pawl].resell.primary
...
RegisterNetEvent("locations:zone:enter", function(group, name)
    if hasResellZoneAccess(group) then
        TriggerEvent("player/setCurrentResellZone", opJobCore.Jobs[opJobCore.JobType.Pawl].resell.secondary)
    end
end)
