...
RegisterNetEvent("inventory:server:UseItemSlot", function(slot)
    ...
    if itemData.type == "weapon" then
        if not ply.state.is_in_hub then
            TriggerClientEvent("op-core:client:weapon:use-weapon", Player.PlayerData.source, itemData)
        end
...
RegisterServerEvent("inventory:server:ResellItem", function(item, amount, resellZone)
    ...
    if item.metadata.expiration ~= nil and exports["op-utils"]:ItemIsExpired(item) then
