...
RegisterNUICallback("player/setItemUsage", function(data, cb)
    SetNuiFocus(false, false)
    TriggerServerEvent("op-core:server:inventory:set-item-usage", data.shortcut, data.slot)
    cb(true)
end)
...
RegisterNUICallback("player/giveItem", function(data, cb)
    ...
    if player ~= -1 and distance < 2.0 then
        ...
        if amount > 1 then
            amount = exports["op-hud"]:Input("Quantité", 5, data.amount)
        end
...
RegisterNUICallback("player/giveMoney", function(data, cb)
    ...
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.0 then
        local amount = exports["op-hud"]:Input("Quantité", 12)
        ...
        if amount and tonumber(amount) > 0 then
            ...
            if playerIdx == -1 then -- Is NPC
                if currentResellZone ~= nil then
                    ...
                    exports["op-hud"]:DrawNotification("Vous n'êtes pas dans une zone de revente", "error")
                end
            else
...
exports("hasPhone", function()
    ...
    if not hasphone then
        exports["op-hud"]:DrawNotification("Vous n'avez pas de téléphone", "error");
        return false
    end

    if LocalPlayer.state.inv_busy then
        exports["op-hud"]:DrawNotification("Action en cours", "error")
        return false
    end

    if PlayerData.metadata["inlaststand"] or PlayerData.metadata["ishandcuffed"] then
        exports["op-hud"]:DrawNotification("Vous ne pouvez pas accéder à votre téléphone", "error")
        return false
    end
