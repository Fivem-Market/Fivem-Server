...
local spawnMicObject = function()
    ...
    AttachEntityToEntity(MicConfig.object, player, GetPedBoneIndex(player, 60309), 0.1, 0.05, 0.0, 20.0, 110.0, 70.0, 1, 0, 0, 0, 2, 1)
end
...
local micOperator = function()
    if MicConfig.enabled then
        ...
        createMicThread()
        exports["op-voip"]:SetPlayerMicrophoneInUse(true)
    else
        if MicConfig.object ~= nil then
            deleteMicObject()
        end
        exports["op-voip"]:SetPlayerMicrophoneInUse(false)
    end
end
