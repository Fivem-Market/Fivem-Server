    ...
    for zoneName, points in pairs(FoodConfig.Zones) do
        ...
        exports["qb-target"]:AddPolyZone(zoneName, points, {
            ...
            onPlayerInOut = function(isIn)
                if isIn and PlayerData.job.id == opJobCore.JobType.Food and PlayerData.job.onduty then
                    currentField = zoneName
                    QBCore.Functions.TriggerCallback("op-jobs:server:food:getFieldHealth", function(health)
            ...
            options = {
                {
                    ...
                    event = "op-jobs:client:food-collect-ingredients",
                    blackoutGlobal = true,
                    blackoutJob = "food",
                    canInteract = function(entity)
                        local hasPermission = opJobCore.Functions.HasPermission("food", opJobCore.JobPermission.Food.Harvest)
        ...
        options = {
            {
                ...
                canInteract = function()
                    return PlayerData.job.id == opJobCore.JobType.Food and not PlayerData.job.onduty
                end,
            },
            {
                type = "server",
                ...
                event = "QBCore:ToggleDuty",
                canInteract = function()
                    return PlayerData.job.id == opJobCore.JobType.Food and PlayerData.job.onduty
                end,
            },
        },
    })
...
--- FARM
---
AddEventHandler("op-jobs:client:food-collect-ingredients", function()
    Citizen.CreateThread(function()
...
FoodJob.Functions.CollectIngredients = function(field)
    ...
    QBCore.Functions.Progressbar("food-collect-ingredients", "Vous récoltez des ingrédients", FoodConfig.Collect.Grape.Duration, false, true,
        ...
        if not wasCancelled then
            QBCore.Functions.TriggerCallback("op-jobs:server:food-collect-ingredients", function(items, newHealth)
                ...
                if next(items) then
                    ...
                    exports["op-hud"]:DrawNotification(string.format("Vous avez récolté ~g~%s", joined))

                    if currentFieldHealth == 0 then
                        exports["op-hud"]:DrawNotification("Le champ est épuisé...", "warning")
                    else
                        TriggerEvent("op-jobs:client:food-collect-ingredients")
                    end
                end
                DisplayFieldHealth(true, currentField, currentFieldHealth)
            end, field)
        else
            exports["op-hud"]:DrawNotification("Vous n'avez pas récolté d'ingrédients", "error")
        end
    end)
end
...
AddEventHandler("jobs:client:food-harvest-milk", function()
    QBCore.Functions.Progressbar("food-harvest-milk", "Vous récupérez des pots de lait", FoodConfig.Collect.Milk.Duration, false, true,
        ...
        QBCore.Functions.TriggerCallback("op-jobs:server:food-collect-milk", function(success, count, item)
            if success then
                exports["op-hud"]:DrawNotification(string.format("Vous avez récupéré ~g~%s pots de lait~s~", count))
                Citizen.Wait(1000)
        ...
        exports["op-hud"]:DrawNotification("Vous avez ~r~interrompu~s~ la collecte de pot de lait", "error")
    end)
end)
...
FoodJob.Functions.GetItemCountFromInventory = function(itemName)
    ...
    for _, item in pairs(PlayerData.items or {}) do
        if item.name == itemName then
            if not exports["op-utils"]:ItemIsExpired(item) then
...
FoodJob.Functions.CraftItem = function(itemId, item)
    if not inKitchen then
        exports["op-hud"]:DrawNotification("Vous n'êtes pas dans la cuisine", "error")
        return
    end

    local recipe = FoodConfig.Recipes[itemId]
    if recipe == nil then
        exports["op-hud"]:DrawNotification("Recette invalide", "error")
        return
    end
    ...
    for ingId, count in pairs(ingredients) do
        ...
        if ingredient == nil then
            exports["op-hud"]:DrawNotification("Ingrédient invalide", "error")
            return
        end
        local countInInv = FoodJob.Functions.GetItemCountFromInventory(ingId) or 0
        if countInInv < count then
            exports["op-hud"]:DrawNotification("Il vous manque des ingrédients", "error")
            return
        end
    end
    ...
    Citizen.CreateThread(function()
        QBCore.Functions.Progressbar("food-craft-item", string.format("Vous préparez %s", item.label),
            ...
            if not wasCancelled then
                QBCore.Functions.TriggerCallback("op-jobs:server:food-craft", function(success, reason)
                    if success then
                        exports["op-hud"]:DrawNotification(string.format("Vous avez préparé ~g~%s", item.label))
                        ...
                        elseif reason == "invalid_ingredient" then
                            ...
                            exports["op-hud"]:DrawNotification(string.format("Vous n'avez pas terminé votre préparation. Il y a eu une erreur : %s", reason),
                                                                "error")
                        end
                    end
                end, itemId)
            else
                exports["op-hud"]:DrawNotification("Vous n'avez pas terminé votre préparation", "error")
            end
        end)
    end)
end

AddEventHandler("op-jobs:client:food-craft-item", function(itemId)
    local item = QBCore.Shared.Items[itemId]
    if item == nil then
        return item
    end
...
RegisterNetEvent("jobs:client:food:hunting", function(data)
    ...
    if HasEntityBeenDamagedByAnyVehicle(data.entity) then
        exports["op-hud"]:DrawNotification("L'animal est tout écrabouillé, on ne pourra rien en tirer...", "warning")
        return
    end
    ...
    QBCore.Functions.Progressbar("hunting-cutup", "Dépeçage en cours...", 5000, false, true,
        ...
        if hasKnife then
            ...
            exports["op-hud"]:DrawNotification("L'animal ne respire plus...", "info")
        end
    end)
end)
...
Citizen.CreateThread(function()
    local resellOpt = opJobCore.Jobs[opJobCore.JobType.Food].resell
