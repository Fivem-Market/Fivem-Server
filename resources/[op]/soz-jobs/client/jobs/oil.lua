local societyMenu = MenuV:CreateMenu(nil, "", "menu_job_fueler", "op", "fueler:menu")
...
local function SpawnFieldZones()
    for field, data in pairs(FuelerConfig.Fields) do
        ...
        fieldZone:onPlayerInOut(function(isInside)
            if isInside and PlayerData.job.id == opJobCore.JobType.Oil and PlayerData.job.onduty then
                ...
                currentField = field
                QBCore.Functions.TriggerCallback("op-jobs:server:fueler:getFieldHealth", function(health)
...
CreateThread(function()
    ...
    exports["qb-target"]:AddBoxZone("mtp:duty", vector3(-230.65, 6088.05, 31.39), 0.10, 3.10, {
        ...
        minZ = 30.39,
        maxZ = 33.39,
    }, {options = opJobCore.Functions.GetDutyActions("oil"), distance = 2.5})
        ...
        options = {
            {
                ...
                icon = "c:fuel/remplir.png",
                event = "op-core:client:oil:update-station-price",
                canInteract = function()
                    return PlayerData.job.onduty and opJobCore.Functions.HasPermission("oil", opJobCore.JobPermission.Fueler.ChangePrice)
                end,
                job = "oil",
            },
        },
...
RegisterNetEvent("jobs:client:fueler:PrepareTankerRefill", function(data)
    ...
    if not lockTanker then
        exports["op-hud"]:DrawNotification("Le tanker ne possède que 2 connexions.", "error")
    ...
    exports["op-hud"]:DrawNotification("Vous cherchez à ~r~connecter~s~ le Tanker.", "info")
...
RegisterNetEvent("jobs:client:fueler:StartTankerRefill", function(data)
    ...
    if not hasInventory then
        exports["op-hud"]:DrawNotification("Le tanker n'a pas d'inventaire.", "error")
    ...
    if LocalPlayer.state.tankerUsing then
        exports["op-hud"]:DrawNotification("Vous utilisez deja le tanker.", "error")
    ...
    exports["op-hud"]:DrawNotification("Vous avez ~g~relié~s~ le Tanker au ~g~Puit de pétrole~s~.", "info")

    while canFillTanker do
        Wait(500)

        local success, _ = exports["op-utils"]:Progressbar("fill", "Vous remplissez...", 24000, false, true, {
    ...
    exports["op-hud"]:DrawNotification("La récolte est ~g~terminée~s~ ! Le tanker a été ~r~déconnecté~s~.", "info")
end)
...
RegisterNetEvent("jobs:client:fueler:StartTankerRefining", function(data)
    ...
    if LocalPlayer.state.tankerUsing then
        exports["op-hud"]:DrawNotification("Vous utilisez déjà le tanker.", "error")
    ...
    exports["op-hud"]:DrawNotification("Vous avez ~g~relié~s~ le Tanker à ~g~la raffinerie~s~.", "info")

    while canRefiningTanker do
        Wait(500)

        local success, _ = exports["op-utils"]:Progressbar("fill", "Vous raffinez...", 24000, false, true, {
            disableMovement = true,
            disableCombat = true,
        }, {animDict = "timetable@gardener@filling_can", anim = "gar_ig_5_filling_can", flags = 1}, {}, {})
    ...
    LocalPlayer.state.tankerUsing = false
    TriggerEvent("jobs:client:fueler:CancelTankerRefill")
    exports["op-hud"]:DrawNotification("Le raffinage est ~g~terminé~s~ ! Le tanker a été ~r~déconnecté~s~.", "info")
end)
...
RegisterNetEvent("jobs:client:fueler:StartCraftEssence", function(data)
    ...
    exports["op-hud"]:DrawNotification("Vous ~g~démarrez~s~ la transformation.", "info")
...
RegisterNetEvent("jobs:client:fueler:StartTankerResell", function(data)
    ...
    exports["op-hud"]:DrawNotification("Vous avez ~g~relié~s~ le Tanker à ~g~la cuve~s~.", "info")

    while canResellTanker do
        Wait(500)

        local success, _ = exports["op-utils"]:Progressbar("resell", "Vous remplissez...", 5000, false, true, {
            disableMovement = true,
            disableCombat = true,
        }, {animDict = "timetable@gardener@filling_can", anim = "gar_ig_5_filling_can", flags = 1}, {}, {})
    ...
    LocalPlayer.state.tankerUsing = false
    TriggerEvent("jobs:client:fueler:CancelTankerRefill")
    exports["op-hud"]:DrawNotification("Le remplissage est ~g~terminée~s~ ! Le tanker a été ~r~déconnecté~s~.", "info")
end)
