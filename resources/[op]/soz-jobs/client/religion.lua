...
exports["qb-target"]:AddBoxZone("job religion", vector3(-766.24, -24.34, 41.07), 1, 1, {
    ...
    options = {
        ...
        },
        {
            ...
            label = "S'équiper",
            job = opJobCore.JobType.Religious,
            blackoutGlobal = true,
...
AddEventHandler("jobs:religion:fix", function(npcPed)
    for _, p in ipairs(PrayersPed) do
        if p == npcPed then
            exports["op-hud"]:DrawNotification("Vous avez déjà parlé à cette personne", "warning")
            return
        end
    end
    ...
    QBCore.Functions.Progressbar("religion_fix", "Vous balancez une info chat...", 10000, false, true,
        ...
        if PrayersCount >= PrayersMax then
            exports["op-hud"]:DrawNotification(string.format("Vous avez prêché %d infos chat dans cette zone", PrayersCount), "warning")
            exports["op-hud"]:DrawNotification("Retournez voir le prêtre", "info")
...
AddEventHandler("jobs:religion:begin", function()
    ...
    TriggerServerEvent("job:set:pole", opJobCore.JobType.Religious)
    OnJob = true
end)
...
AddEventHandler("jobs:religion:tenue", function()
    QBCore.Functions.Progressbar("switch_clothes", "Changement d'habits...", 5000, false, true, {
        ...
        TriggerServerEvent("op-character:server:SetPlayerJobClothes", opJobCore.religion_clothes[PlayerData.skin.Model.Hash])
...
AddEventHandler("jobs:religion:vehicle", function()
    ...
    TriggerServerEvent("op-core:server:vehicle:free-job-spawn", "fixter", {
        opJobCore.religion_vehicule.x,
        opJobCore.religion_vehicule.y,
        opJobCore.religion_vehicule.z,
        opJobCore.religion_vehicule.w,
...
AddEventHandler("jobs:religion:vehicle-spawn", function(vehicleNetId)
    ...
    JobVehicle = true
    createblip("Véhicule", "Vélo de service", 225, opJobCore.religion_vehicule)
...
local function random_coord()
    local result = opJobCore.religion[math.random(#opJobCore.religion)]
...
AddEventHandler("jobs:religion:start", function()
    ...
    ResetPrayersState()
    PrayersMax = math.random(opJobCore.religion_prayers_range.min, opJobCore.religion_prayers_range.max)
...
local function close()
    ...
    local money = opJobCore.religion_payout * payout_counter
