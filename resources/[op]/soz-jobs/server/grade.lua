RegisterServerEvent("job:recruit", function(target)
    ...
    if not CheckPlayerJobPermission(player.PlayerData, player.PlayerData.job.id, opJobCore.JobPermission.ManageGrade) and
        not CheckPlayerJobPermission(player.PlayerData, player.PlayerData.job.id, opJobCore.JobPermission.Enrollment) then
        return
    end

    local targetPlayer = QBCore.Functions.GetPlayer(tonumber(target))

    if targetPlayer.PlayerData.job.id ~= opJobCore.JobType.Unemployed then
...
RegisterServerEvent("job:fire", function(target)
    local source = source
    local player = QBCore.Functions.GetPlayer(tonumber(source))
    local playerGradeWeight = opJobCore.Jobs[player.PlayerData.job.id].grades[tostring(player.PlayerData.job.grade)].weight
    ...
    local targetPlayer = QBCore.Functions.GetPlayer(tonumber(target))
    local targetGradeWeight = opJobCore.Jobs[targetPlayer.PlayerData.job.id].grades[tostring(targetPlayer.PlayerData.job.grade)].weight
    ...
    targetPlayer.Functions.SetJob(opJobCore.JobType.Unemployed, GetJobDefaultGrade(opJobCore.JobType.Unemployed))
...
RegisterServerEvent("job:promote", function(target, gradeId)
    ...
    local grade = opJobCore.Jobs[targetPlayer.PlayerData.job.id].grades[tostring(gradeId)]
...
RegisterServerEvent("job:grade:add", function(name)
    ...
    if not CheckPlayerJobPermission(player.PlayerData, player.PlayerData.job.id, opJobCore.JobPermission.ManageGrade) then
        return
    end

    local job = opJobCore.Jobs[player.PlayerData.job.id]
