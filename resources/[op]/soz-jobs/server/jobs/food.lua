...
QBCore.Functions.CreateCallback("op-jobs:server:food:getFieldHealth", function(source, cb, fieldName)
...
local function AddItem(source, item, itemCount)
    ...
    exports["op-inventory"]:AddItem(source, item, count, nil, nil, function(success, reason)
        if success then
            receivedItem = true
        else
...
QBCore.Functions.CreateCallback("op-jobs:server:food-collect-ingredients", function(source, cb, fieldName)
...
QBCore.Functions.CreateCallback("op-jobs:server:food-collect-milk", function(source, cb, hour)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then
        return
    end
    ...
    local pollutionLevel = exports["op-upw"]:GetPollutionLevel()
...
QBCore.Functions.CreateCallback("op-jobs:server:food-craft", function(source, cb, itemId)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then
        return
    end
    ...
    for ingId, count in pairs(ingredients) do
        ...
        local ingItem = exports["op-inventory"]:GetItem(source, ingId, nil)
        if ingItem.amount < count or exports["op-utils"]:ItemIsExpired(ingItem) then
            cb(false, "invalid_ingredient")
            return
        end
    end

    exports["op-inventory"]:AddItem(source, itemId, recipe.output or 1, nil, nil, function(success, reason)
        if not success then
...
RegisterNetEvent("jobs:server:food:hunting", function(huntId)
    ...
    for item, reward in pairs(FoodConfig.HuntingReward) do
        ...
        exports["op-inventory"]:AddItem(Player.PlayerData.source, item, amount, nil, nil, function(success, reason)
            if success then
...
QBCore.Functions.CreateUseableItem("meal_box", function(source, item)
    if exports["op-inventory"]:CanCarryItems(source, {
        ...
        exports["op-inventory"]:RemoveItem(source, "meal_box", 1)
        exports["op-inventory"]:AddItem(source, "vegan_meal", 5, item.metadata)
        exports["op-inventory"]:AddItem(source, "onigiri_assortment", 5, item.metadata)
        exports["op-inventory"]:AddItem(source, "meat_festival", 5, item.metadata)
        exports["op-inventory"]:AddItem(source, "royal_vegetables", 5, item.metadata)
    else
