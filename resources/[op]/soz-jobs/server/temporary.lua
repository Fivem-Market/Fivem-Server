RegisterServerEvent("job:set:unemployed", function()
    ...
    Player.Functions.SetJob(opJobCore.JobType.Unemployed, nil)
end)
...
RegisterServerEvent("job:get:metal", function(amount)
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
    local metadata = {}
    exports["op-inventory"]:AddItem(Player.PlayerData.source, "metalscrap", amount, metadata, nil, function(success, reason)
        if success then
...
RegisterServerEvent("job:remove:metal", function(amount)
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
    local totalAmount = exports["op-inventory"]:GetItem(Player.PlayerData.source, "metalscrap", nil, true)
    if tonumber(amount) <= tonumber(totalAmount) then
        exports["op-inventory"]:RemoveItem(Player.PlayerData.source, "metalscrap", amount, nil)
        TriggerClientEvent("hud:client:DrawNotification", source, string.format("Vous avez vendu %d ferrailles", amount))
        local payout = amount * opJobCore.metal_payout
