    ...
    local bankActions = {
        ...
        },
        {
            ...
            canInteract = function(entity, distance, data)
                return PlayerData.job.onduty and opJobCore.Functions.HasPermission(PlayerData.job.id, opJobCore.JobPermission.SocietyBankAccount) and
    ...
    for _, item in pairs({"small_moneybag", "medium_moneybag", "big_moneybag"}) do
        table.insert(bankActions, {
            ...
            action = function()
                ...
                TriggerServerEvent("op-core:server:job:stonk:fill-in", currentBank.bank, item, currentMoney, maxMoney)
            end,
            item = item,
        })
    end
    ...
    local function atmRefillAction(atmType, item)
        return {
            ...
            action = function(entity)
                ...
                TriggerServerEvent("op-core:server:job:stonk:fill-in", atm.account, item, currentMoney, maxMoney)
            end,
            item = item,
        }
    end
    ...
    if data.safe.owner == nil or (PlayerData.job ~= nil and PlayerData.job.id == data.safe.owner) then
        QBCore.Functions.TriggerCallback("banking:server:openSafeStorage", function(isAllowed, money, black_money)
            if isAllowed then
                ...
                exports["op-hud"]:DrawNotification("Vous n'avez pas accès à ce coffre", "error")
            end
        end, data.SafeId)
    end
end)
