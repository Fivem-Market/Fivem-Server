...
local function PlayerHaveAccessToInvoices(PlayerData, account)
    ...
    return opJobCore.Functions.HasPermission(account, PlayerData.job.id, PlayerData.job.grade, opJobCore.JobPermission.SocietyBankInvoices)
end
...
local function CreateInvoice(Emitter, Target, account, targetAccount, label, amount, kind)
    ...
    local id = MySQL.insert.await(
            ...
            opJobCore.Jobs[Emitter.PlayerData.job.id].label,
    ...
    if id then
        ...
        Invoices[targetAccount][id] = {
            ...
            emitterName = opJobCore.Jobs[Emitter.PlayerData.job.id].label,
...
RegisterNetEvent("banking:server:sendInvoice", function(target, label, amount, kind)
    ...
    if exports["op-inventory"]:RemoveItem(Player.PlayerData.source, "paper", 1) then
