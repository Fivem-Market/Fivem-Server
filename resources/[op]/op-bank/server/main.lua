...
QBCore.Functions.CreateCallback("banking:getBankingInformation", function(source, cb, account)
    ...
    if Player then
        if account == nil then
            ...
            elseif opJobCore.Functions.HasPermission(account, Player.PlayerData.job.id, Player.PlayerData.job.grade,
                                                      opJobCore.JobPermission.SocietyBankAccount) then
                account = Account(account)
            else
                cb(nil)
                return
...
QBCore.Functions.CreateCallback("banking:server:createOffshoreAccount", function(source, cb, account)
    ...
    if opJobCore.Functions.HasPermission(account, Player.PlayerData.job.id, Player.PlayerData.job.grade, opJobCore.JobPermission.SocietyBankAccount) then
        if offshore == nil then
...
QBCore.Functions.CreateCallback("banking:server:TransfertOffshoreMoney", function(source, cb, accountTarget, amount)
    ...
    if opJobCore.Functions.HasPermission(accountTarget, Player.PlayerData.job.id, Player.PlayerData.job.grade, opJobCore.JobPermission.SocietyBankAccount) then
...
QBCore.Functions.CreateCallback("banking:server:openHouseSafeStorage", function(source, cb, safeStorage)
    ...
    local apartmentTier = exports["op-housing"]:GetApartmentTier(inside.property, inside.apartment)
    local account = Account(safeStorage)
