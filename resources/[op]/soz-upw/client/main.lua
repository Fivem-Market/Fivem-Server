...
RegisterNetEvent("QBCore:Client:OnPlayerLoaded", function()
    ...
    exports["qb-target"]:AddBoxZone("upw:duty", vector3(582.72, 2756.97, 41.86), 0.4, 0.3, {
        ...
        minZ = 42.10,
        maxZ = 42.60,
    }, {options = opJobCore.Functions.GetDutyActions("upw"), distance = 2.5})
end)
...
Citizen.CreateThread(function()
    ...
    local facilities = QBCore.Functions.TriggerRpc("op-upw:server:GetFacilitiesFromDb", types)
...
RegisterNetEvent("op-upw:client:CreateZone", function(identifier, zoneType, zone, data)
    local conf = GetZoneConfig(zoneType)

    if conf.create then
        conf.create(identifier, zone, data)
    else
        exports["op-hud"]:DrawNotification("Erreur lors de la création de la zone", "error")
    end
end)
