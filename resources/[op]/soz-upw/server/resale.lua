QBCore.Functions.CreateCallback("op-upw:server:ResaleEnergy", function(source, cb)
    ...
    for _, energyItem in pairs(Config.Items.Energy) do
        ...
        local count = exports["op-inventory"]:GetItem(Player.PlayerData.source, item.name, nil, true)
        if count == 0 then
            goto continue
        end

        if exports["op-inventory"]:RemoveItem(Player.PlayerData.source, item.name, 1) then
