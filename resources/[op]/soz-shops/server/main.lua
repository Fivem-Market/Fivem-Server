...
RegisterNetEvent("shops:server:pay", function(brand, product, amount)
    ...
    if Player then
        ...
        if shouldCheckAmount(brand) then
            ...
            local canCarryItem = exports["op-inventory"]:CanCarryItem(Player.PlayerData.source, qbItem, amount)
        ...
        if Player.Functions.RemoveMoney("money", price) then
            ...
            elseif brand == "ponsonbys" or brand == "suburban" or brand == "binco" then
                ...
                exports["op-inventory"]:AddItem(Player.PlayerData.source, item.name, amount, nil, nil, function(success, reason)
                    if success then
...
RegisterNetEvent("shops:server:CheckZkeaStock", function()
    ...
    if player then
        local amount = exports["op-inventory"]:GetItem("cabinet_storage", "cabinet_zkea", nil, true)
