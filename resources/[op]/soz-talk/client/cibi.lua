...
local function handleUpdateRadio(data, isPrimary)
    ...
    if isPrimary then
        exports["op-voip"]:SetRadioLongRangePrimaryVolume(data.volume or 100)
        TriggerEvent("voip:client:radio:set-balance", "radio-lr", "primary", data.ear)
    else
        exports["op-voip"]:SetRadioLongRangeSecondaryVolume(data.volume or 100)
...
local function toggleRadio(toggle)
    if toggle and Entity(currentVehicle).state.radioInUse then
        exports["op-hud"]:DrawNotification("La radio est déjà utilisée", "error")
