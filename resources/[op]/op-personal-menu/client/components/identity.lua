...
function GenerateIdentityMenu(identitySubmenu, selectedCheckbox, showingAround)
    ...
    local checkboxes = {
        ["identity"] = {label = "carte d'identité", event = "op-identity:client:request-identity-data"},
        ["licenses"] = {label = "permis", event = "op-identity:client:request-licenses-data"},
        ["health_book"] = {label = "carte de santé", event = "op-core:client:player:health:request-health-book"},
    }
                        ...
                        if type(closePlayers) == "table" and #closePlayers > 1 then
                            ...
                            for _, player in ipairs(closePlayers) do
                                if player ~= PlayerId() then
                                    ...
                                    TriggerEvent("op-identity:client:give-animation")
                                end
                            end

                        else
                            exports["op-hud"]:DrawNotification("Il n'y a personne à proximité", "error", 3000)
...
function HideUI(identitySubmenu)
    TriggerEvent("op-identity:client:hide")
    selectedCheckbox = nil
    if showingAround then
        TriggerServerEvent("op-identity:server:hide-around", showingTo)
...
AddEventHandler("op-personal-menu:client:identity:resetMenu", function()
