function JobEntry(menu)
    ...
    if not opJobCore.Functions.HasPermission(PlayerData.job.id, opJobCore.JobPermission.ManageGrade) then
        return
    end
    ...
    gradeMenu:AddButton({label = "Ajouter un grade", value = "add_grade"}):On("select", function()
        local gradeName = exports["op-hud"]:Input("Nom du grade :", 32)
    ...
    for gradeId, grade in pairs(job.grades) do
        ...
        gradeItemMenu:AddButton({label = "Changer le salaire (" .. grade.salary .. ")", value = "set_salary"}):On("select", function()
            local salary = exports["op-hud"]:Input("Nouveau salaire :", 32)
        ...
        gradeItemMenu:AddButton({label = "Changer l'importance (" .. grade.weight .. ")", value = "set_weight"}):On("select", function()
            local weight = exports["op-hud"]:Input("Importance (le patron doit être le plus élevé) :", 32)
